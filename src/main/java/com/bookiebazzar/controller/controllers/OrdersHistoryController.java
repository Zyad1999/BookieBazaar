...
public class OrdersHistoryController  extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if(req.getParameter("id") != null && currentUser.isAdmin()){
            ...
            listOfOrder = OrderServicesImpl.getOrderServices().getListOfOrderDto( Integer.parseInt(req.getParameter("id")) , (EntityManager)req.getAttribute("entityManager"));
        } else{
            listOfOrder = OrderServicesImpl.getOrderServices().getListOfOrderDto(((UserDto) session.getAttribute("currentUser")).getId(), (EntityManager)req.getAttribute("entityManager"));
        }    
